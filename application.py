...
def index():
    ...
    hs = db.execute("SELECT b.ticker, sh.company, IFNULL((b.num_shares-s.num_shares),b.num_shares) AS num_shares FROM buy b JOIN shares sh ON b.ticker=sh.ticker LEFT JOIN sell s ON b.cuid=s.cuid AND b.ticker=s.ticker where b.cuid=:cuid GROUP BY b.ticker, sh.company ORDER BY sh.company", uid)
    # the query returns row objects, convert them to dict objects using list comprehension
    hold_share = [dict(row) for row in hs.fetchall()]
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(check_username) > 0:
            ...
            h_pass0 = generate_password_hash(request.form.get("password"))
            hp = dict(h_pass=h_pass0)
            db.execute("INSERT INTO users ('username', 'hash') VALUES (:username, :hashed_pass)", (username["username"], hp["h_pass"]))
